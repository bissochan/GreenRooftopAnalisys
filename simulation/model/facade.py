...
def compute_facade_effects(T_air_end_c, T_air_end_g, T_env, wind, H_mix):
    ...
    T_air_loc_c = T_air_end_c * (1 - relax_factor) + T_env * relax_factor
    T_air_loc_g = T_air_end_g * (1 - relax_factor) + T_env * relax_factor

    a_z_c = g * max(T_air_loc_c - T_env, 0.0) / (T_env + T0_K)
    a_z_g = g * max(T_air_loc_g - T_env, 0.0) / (T_env + T0_K)
