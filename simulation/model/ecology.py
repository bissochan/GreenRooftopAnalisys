...
def compute_ecology(radiation, co2, co, pm10, pm25, air_flow_m3_per_h=None):
    """
    Compute ecological removals for one hour step.
    air_flow_m3_per_h: airflow over roof (m³/h). Uses ventilation if provided.
    """
    # CO2 uptake (g/h) 
    ...
    if air_flow_m3_per_h is not None and air_flow_m3_per_h > 0:
        # Ventilation: mass = C * Q (µg/m³ * m³/h = µg/h)
        pm10_removed_ug = pm10_c * air_flow_m3_per_h
        pm25_removed_ug = pm25_c * air_flow_m3_per_h  
        co_removed_ug = co_c * air_flow_m3_per_h
    else:
        # Deposition (con cap fisico)
